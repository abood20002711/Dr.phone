...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
    ...
    $sql = "select * from myshop.Category where CategoryName=:CategoryName";
    $stm2= $con->prepare($sql);
    $stm2->bindParam(':CategoryName', $CategoryName);
    $stm2 -> execute();
    
    $sql2= "select * from myshop.Category where CategoryCode=:CategoryCode";
    $stm3= $con->prepare($sql2);
    $stm3->bindParam(':CategoryCode', $CategoryCode);
    $stm3 -> execute();
    if ($stm2->rowCount() > 0 or $stm3->rowCount() > 0) {
        echo "<script>     Swal.fire({
        title: 'Error!',
        text: 'The Category Code  or Category Name already exists, use another Product Code ',
        icon: 'error',
        confirmButtonText: 'OK'
      })</script>";
    } 
    else{
    $stmt= $con->prepare("insert into myshop.Category (CategoryName,CategoryCode,Description,parentID) VALUES 
    (:CategoryName,:CategoryCode,:Description,:parentID)");
                            ...
                            <div class="row">
                                <div class="col-lg-6 col-sm-6 col-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="CategoryName" id="" class="form-control " required placeholder="Category Name">
                                    </div>
                                </div>
                                <div class="col-lg-6 col-sm-6 col-12">
                                    <div class="form-group">
                                        <label for="">Category Code *</label>
                                        <input type="text" name="CategoryCode" pattern="[0-9-()-+ ]*" id="" class="form-control" required placeholder="Use Numbers">
                                    </div>
                                </div>
                                        ...
                                        <select name="parentID" id="" class="form-control">
                                            <option value="">Select Main Category</option>
                                                    ...
                                                    foreach ($result as $cat) {
                                                        echo "<option value='". $cat['MainCatID'] ."' >". $cat['NameMainCat'] ."</option>" ;
